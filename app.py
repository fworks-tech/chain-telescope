...
with st.sidebar:
  ...
  st.radio("Nav", ["Dashboard","Alerts","News","Risk","Newsletter"], label_visibility="collapsed")
...
c1.markdown('<div class="card"><div class="small">Market Cap Tracked</div><div class="val">$2.14T</div><div style="color:#12b76a">+2.8%</div></div>', unsafe_allow_html=True)
c2.markdown('<div class="card"><div class="small">24H Volume</div><div class="val">$86.3B</div><div style="color:#12b76a">+5.1%</div></div>', unsafe_allow_html=True)
c3.markdown('<div class="card"><div class="small">Risk Index</div><div class="val">62 / 100</div><div style="color:#f79009">Elevated</div></div>', unsafe_allow_html=True)
c4.markdown('<div class="card"><div class="small">Active Alerts</div><div class="val">7</div><div style="color:#3b5bff">3 triggered today</div></div>', unsafe_allow_html=True)
...
with right:
  ...
  rfig=go.Figure(data=[go.Bar(
    x=[62,48,71,55], y=["Market","Liquidity","Momentum","Whale Activity"], orientation="h",
...
if st.button("Subscribe"):
  st.success(f"Subscribed: {email} • {freq} • {fmt}") if "@" in email and "." in email else st.error("Please enter a valid email.")
st.markdown('</div>', unsafe_allow_html=True)
