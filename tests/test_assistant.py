...
class AssistantUnitTests(unittest.TestCase):
  ...
  def test_smart_context_search_returns_relevant_and_excludes_irrelevant_items(self):
    ...
    self.assertTrue(any("alert" in item.lower() for item in matches) or any("alert" in item.lower() for item in context["alerts"]))
    self.assertTrue(any("ETF" in item for item in matches) or any("ETF" in item for item in context["news"]))
